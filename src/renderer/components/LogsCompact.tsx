...
const LogItem = memo(function LogItem({ log }: { log: LogEntry }) {
  ...
  return (
    <div className="flex gap-2 py-1 border-b border-gray-200 dark:border-slate-800/50 text-xs">
      <span className="text-gray-400 dark:text-slate-500 shrink-0">[{formatTimestamp(new Date(log.timestamp))}]</span>
      <span className="shrink-0">{config.icon}</span>
      <span className={`${config.color} truncate`}>{log.message}</span>
    </div>
  );
});
