...
import { isSpecialAccountCode } from '../CSVEditorTab.utils';
...
const ExtrasEditorModal = memo(function ExtrasEditorModal({
  ...
  const extAccountCodes = Object.keys(mappings).filter((code) => !isSpecialAccountCode(code));

  const handleEntryChange = (index: number, field: keyof ExtDraftEntry, value: string) => {
    const next = [...entries];
    
    if (field === 'cuenta') {
      const nextCuenta = value.toUpperCase();
      const projectKeys = nextCuenta && mappings[nextCuenta]?.projects
        ? Object.keys(mappings[nextCuenta].projects)
        : [];
      const prevProyecto = next[index]?.proyecto ?? '';
    ...
    } else if (field === 'proyecto') {
      next[index] = { ...next[index], proyecto: value.toUpperCase() };
  ...
  const handleAddEntry = () => {
    onEntriesChange([...entries, { cuenta: '', proyecto: '', inicio: '', fin: '' }]);
  };

  const handleRemoveEntry = (index: number) => {
    const next = entries.filter((_, idx) => idx !== index);
    onEntriesChange(next.length ? next : [{ cuenta: '', proyecto: '', inicio: '', fin: '' }]);
  };
        ...
        <div className="overflow-auto">
          <table className="w-full text-sm" role="grid" aria-label={t('extrasEditor.title')}>
            <thead className="text-gray-500 dark:text-slate-400 border-b border-gray-200 dark:border-slate-700">
              <tr>
                <th className="text-left py-2">{t('csvEditor.account')}</th>
                <th className="text-left py-2">{t('csvEditor.project')}</th>
            ...
            <tbody>
              {entries.map((entry, i) => {
                const projectCodes = entry.cuenta && mappings[entry.cuenta]?.projects
                  ? Object.keys(mappings[entry.cuenta].projects)
                  : [];
                ...
                return (
                  <tr key={i} className="border-b border-gray-100 dark:border-slate-700/50">
                    ...
                    <td className="py-2 pr-2">
                      <select
                        value={entry.cuenta}
                        onChange={(e) => handleEntryChange(i, 'cuenta', e.target.value)}
                        ...
                        title={t('csvEditor.account')}
                        aria-label={`${t('csvEditor.account')} ${i + 1}`}
                      >
                    ...
                    <td className="py-2 pr-2">
                      <select
                        value={entry.proyecto}
                        onChange={(e) => handleEntryChange(i, 'proyecto', e.target.value)}
                        className="w-full bg-gray-50 dark:bg-dark-300 border border-gray-300 dark:border-slate-600 rounded-lg text-gray-900 dark:text-white disabled:opacity-50"
                        disabled={!entry.cuenta || !mappings[entry.cuenta]}
                        title={t('csvEditor.project')}
                        aria-label={`${t('csvEditor.project')} ${i + 1}`}
                      >
                        <option value="">{t('csvEditor.selectProject')}</option>
                        {projectCodes.map((code) => (
                          <option key={code} value={code}>
                            {code} - {mappings[entry.cuenta].projects[code] || 'N/A'}
                          </option>
                        ))}
                      </select>
                    </td>
