/**
 * PDF Report Service - Generación de reportes PDF
 */
...
<body>
  ...
  /**
   * Generate PDF report (saves as HTML for now, can be printed to PDF)
   */
  ...
  listReports(): Array<{ name: string; path: string; createdAt: Date }> {
    try {
      ...
      console.error('[PdfReport] Error listing reports:', error);
      return [];
    }
  }
  ...
  deleteReport(filePath: string): boolean {
    try {
      ...
      console.error('[PdfReport] Error deleting report:', error);
      return false;
    }
  }
}
